...
document.addEventListener('DOMContentLoaded', () => {
  renderAuth();
  updateBadge();
  checkTokenExpiry();           // verificar token al cargar
...
//
//  Uso:  const r = await apiFetch('/shopping/cart/items', { method:'POST', body:... })
//
//  Qué hace:
//  1. Agrega automáticamente Authorization: Bearer <token>
//  2. Si la respuesta es 401 → token expirado → logout automático
//     y muestra modal de login con mensaje claro
//  3. Si no hay token, lanza error inmediatamente (no llama al API)
//  4. Pasa cualquier opción extra a fetch normalmente
// ═══════════════════════════════════════════════════════════════
async function apiFetch(path, options = {}) {
  if (!token) {
    openModal('login');
    toast('Tu sesión ha expirado. Inicia sesión de nuevo.', '🔒');
...
function handleTokenExpired() {
  // Limpiar estado sin hacer toast de "sesión cerrada" (confunde al usuario)
  ...
  // Mostrar toast explicativo ANTES de abrir el modal
  toast('Tu sesión expiró. Por favor inicia sesión de nuevo.', '🔒');

  // Pequeño delay para que el toast sea visible antes del modal
  setTimeout(() => {
    openModal('login');
...
// El JWT tiene la fecha de expiración en el payload (campo "exp")
function checkTokenExpiry() {
  ...
  try {
    ...
    if (remainingMs <= 0) {
      // Ya expiró — limpiar silenciosamente sin abrir modal
      token = null; user = null; cart = [];
      localStorage.removeItem('ss_token');
      localStorage.removeItem('ss_user');
      localStorage.removeItem('ss_cart');
    ...
    // (máximo 24h para no dejar timers colgados indefinidamente)
    const delay = Math.min(remainingMs, 24 * 60 * 60 * 1000);
    setTimeout(() => {
      if (token) handleTokenExpired();
    }, delay);
...
function toast(msg, icon='✓') {
  const iEl = document.getElementById('t-icon');
  const mEl = document.getElementById('t-msg');
...
function showColdBanner(attempt) {
  ...
  el.innerHTML = `
    <div class="cold-inner">
      ...
      <div class="cold-body">
        ...
        <div class="cold-sub">${isRetry ? 'El servidor tardó demasiado. Intentando de nuevo automáticamente.' : 'Render apaga el servidor tras 15 min de inactividad. La primera carga puede tomar hasta 30 segundos.'}</div>
      </div>
      <button class="cold-retry" onclick="manualRetry()">Reintentar ahora</button>
      <button class="cold-close" onclick="hideColdBanner()" title="Cerrar">×</button>
    </div>
  ...
  if (bar) {
    ...
    requestAnimationFrame(() => {
      bar.style.transition = `width ${FETCH_TIMEOUT}ms linear`;
      bar.style.width = '100%';
    });
  }
}
...
function manualRetry() {
  _retryCount = 0; hideColdBanner();
  loadProducts(activeFilter === 'ALL' ? '' : activeFilter);
}
...
async function loadProducts(name='', cat='') {
  ...
  while (_retryCount <= MAX_RETRY) {
    try {
      ...
      // loadProducts usa fetch simple (es público, no requiere token)
      const r = await fetch(url, { signal: AbortSignal.timeout(FETCH_TIMEOUT) });
      if (!r.ok) throw new Error(`HTTP ${r.status}`);
      const d = await r.json();
      allProds = (d.content || d.products || (Array.isArray(d) ? d : []))
        .map(p => ({ ...p, _stars:rng(3,5), _reviews:rng(18,340) }));
      success = true; _retryCount = 0; break;
    } catch {
      _retryCount++;
      if (_retryCount > MAX_RETRY) break;
      showColdBanner(_retryCount);
      await new Promise(res => setTimeout(res, 2000));
    }
  }
  clearTimeout(_coldTimer); _coldTimer = null;
  if (success) { hideColdBanner(); _retryCount = 0; }
  else { loadMock(); showFallbackBanner(); }
...
function showFallbackBanner() {
  ...
  el.innerHTML = `
    <div class="cold-inner">
      ...
      <div class="cold-body">
        <div class="cold-title">No se pudo conectar con el servidor</div>
        <div class="cold-sub">Mostrando productos de ejemplo. Los datos reales no están disponibles.</div>
      </div>
...
function updateCounts() {
  ...
  safe('cnt-all', allProds.length);
  safe('cnt-fw',  allProds.filter(p=>p.category==='FOOTWEAR').length);
  safe('cnt-cl',  allProds.filter(p=>p.category==='CLOTHING').length);
  safe('cnt-eq',  allProds.filter(p=>p.category==='EQUIPMENT').length);
}
...
function renderSkeleton() {
  ...
  g.innerHTML = Array(8).fill(0).map(()=>`
    <div class="pcard">
      <div class="pcard-img sk" style="aspect-ratio:1;background:var(--paper2)"></div>
      <div class="pcard-body" style="gap:.5rem">
        ...
        <div class="sk" style="height:32px;width:100%;margin-top:.25rem"></div>
      </div>
    </div>`).join('');
}
...
function renderGrid(prods) {
  const g = document.getElementById('pgrid'); if(!g) return;
  if (!prods.length) {
    g.innerHTML=`<div class="empty"><div class="empty-icon">🔍</div><div class="empty-title">Sin resultados</div><div class="empty-sub">Prueba con otros filtros o una búsqueda diferente.</div></div>`;
    return;
  }
  g.innerHTML = prods.map(p => {
    const low    = p.stock>0 && p.stock<=3;
    const isNew  = p.id%5===0;
    const isSale = p.id%4===0;
    let badge='';
    ...
    return `
    <div class="pcard" onclick="goToProduct(${p.id})" style="cursor:pointer">
      ${badge}
      <div class="pcard-img"><span>${ICONS[p.category]||'📦'}</span></div>
      <button class="wish-btn" onclick="event.stopPropagation();toast('Guardado en favoritos','❤️')" title="Guardar">
      ...
      <div class="pcard-body">
        ...
        <div class="pcard-stars">
          <div class="stars">${stars(p._stars)}</div>
          <span class="review-n">(${p._reviews})</span>
        </div>
        ${low?`<div class="stock-warn">⚡ Solo ${p.stock} disponibles</div>`:''}
        <div class="pcard-foot">
          <div class="pcard-price">${fmt(p.price)}</div>
          ${p.stock>0
            ?`<button class="add-btn" onclick="addToCart(event,${p.id},'${p.name.replace(/'/g,"\\'")}',${p.price},'${p.category}')">
...
// ── FILTERS ──────────────────────────────────────────────────
function setFilter(cat, el, isMobile=false) {
  activeFilter = cat;
  const listId = isMobile ? 'cat-list-m' : 'cat-list';
  document.querySelectorAll(`#${listId} .cat-opt`).forEach(c=>c.classList.remove('on'));
  el.classList.add('on');
...
function onStockToggle() {
  const d=document.getElementById('stock-toggle'), m=document.getElementById('stock-toggle-m');
  onlyStock = (d&&d.checked)||(m&&m.checked);
  if(d) d.checked=onlyStock; if(m) m.checked=onlyStock;
  applyFilters();
}
...
function onSearch(v) {
  clearTimeout(_st);
  _st = setTimeout(()=>{ v.length>1 ? loadProducts(v,'') : loadProducts('',activeFilter); }, 380);
}
...
function renderChips() {
  const row=document.getElementById('chips-row'); if(!row) return;
  _chips=[];
  ...
  row.innerHTML=_chips.length
    ?_chips.map((c,i)=>`<div class="chip">${c.label}<button class="chip-x" onclick="_chips[${i}].clear()">×</button></div>`).join('')+`<button class="clear-btn" onclick="clearAll()">Limpiar todo</button>`
    :'';
}
function clearAll() {
  activeFilter='ALL';onlyStock=false;maxPrice=300;activeSort='';
  const sl=document.getElementById('price-slider');if(sl)sl.value=300;
  const pl=document.getElementById('price-lbl');if(pl)pl.textContent='S/ 300';
  ['stock-toggle','stock-toggle-m'].forEach(id=>{const el=document.getElementById(id);if(el)el.checked=false;});
  document.querySelectorAll('#cat-list .cat-opt,#cat-list-m .cat-opt').forEach((c,i)=>c.classList.toggle('on',i===0));
  loadProducts('','ALL');
}
...
function updateBadge() {
  ...
  if(cn) cn.textContent=`${n} item${n!==1?'s':''}`;
...
function addToCart(e, id, name, price, cat, qty=1) {
  ...
  // ← apiFetch reemplaza fetch + Authorization manual
  apiFetch('/shopping/cart/items', {
    method:'POST',
    body: JSON.stringify({productId:id, quantity: ex?ex.qty:qty})
  }).catch(()=>{});
}
...
function renderCart() {
  const body=document.getElementById('cart-body'), ft=document.getElementById('cart-ft');
  if(!body) return;
  if(!cart.length) {
    body.innerHTML=`<div class="cart-empty"><div class="cart-empty-icon">🛒</div><div class="cart-empty-title">Tu carrito está vacío</div><div class="cart-empty-sub">Agrega productos para comenzar tu pedido.</div></div>`;
    if(ft) ft.style.display='none'; return;
  }
  body.innerHTML=cart.map(i=>`
    <div class="ci">
      <div class="ci-thumb">${ICONS[i.cat]||'📦'}</div>
      <div class="ci-body">
        <div class="ci-name">${i.name}</div>
        <div class="ci-brand">${fmt(i.price)} c/u</div>
        <div class="ci-row">
          <div class="qty-ctrl">
            <button class="q-btn" onclick="chgCartQty(${i.id},-1)">−</button>
            <span class="q-n">${i.qty}</span>
            <button class="q-btn" onclick="chgCartQty(${i.id},1)">+</button>
          </div>
          <div class="ci-price">${fmt(i.price*i.qty)}</div>
        </div>
        <button class="ci-del" onclick="removeItem(${i.id})">
...
function chgCartQty(id, d) {
  ...
  // ← apiFetch reemplaza fetch + Authorization manual
  if(it.qty>0) {
    apiFetch(`/shopping/cart/items/${id}`, {
      method:'PUT', body:JSON.stringify({quantity:it.qty})
    }).catch(()=>{});
  } else {
    apiFetch(`/shopping/cart/items/${id}`, { method:'DELETE' }).catch(()=>{});
  }
}
...
function renderAuth() {
  ...
  if (user) {
    el.innerHTML = `
      <div class="auth-chip">
        <div class="dot"></div>
        <span style="font-weight:500">${user.firstName || 'Usuario'}</span>
        <a href="orders.html" style="font-size:12px;color:var(--ink4);margin-left:4px;text-decoration:none;font-family:Outfit,sans-serif;transition:color .2s" onmouseover="this.style.color='var(--ink)'" onmouseout="this.style.color='var(--ink4)'">pedidos</a>
        <button onclick="logout()" style="background:none;border:none;cursor:pointer;font-size:12px;color:var(--ink4);margin-left:2px;font-family:Outfit,sans-serif" onmouseover="this.style.color='var(--ink)'" onmouseout="this.style.color='var(--ink4)'">salir</button>
      </div>`;
  } else {
    el.innerHTML = `
...
function showMsg(id, txt, type) {
  ...
  el.textContent = txt; el.className='modal-msg '+(type||'err'); el.style.display='block';
}
...
async function doLogin() {
  ...
  try {
    ...
    token = d.token;
    user  = d.user || { firstName: em.split('@')[0] };
    localStorage.setItem('ss_token', token);
    localStorage.setItem('ss_user', JSON.stringify(user));
    checkTokenExpiry();           // programar el logout automático con el nuevo token
